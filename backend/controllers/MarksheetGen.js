const MarksheetGen = require('../models/MarksheetGen');
...
const createMarksheet = async (req, res) => {
    ...
    const groupName = req.body.GroupName;
        ...
        // Check if marks already exist for the selected group and assignment
        const existingMarksheet = await MarksheetGen.findOne({ assID: assID, GroupName: groupName });

        if (existingMarksheet) {
            return res.status(400).json({ message: 'Marks already exist for this group and assignment' });
        }
...
const deleteMarksheet = async (req, res) => {
    ...
    try {
        // Find and delete the marksheet by ID
        const deletedMarksheet = await MarksheetGen.findByIdAndDelete(id);
